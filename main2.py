...
contagem_classificacoes_necessarias = {
    ...
    "Primeiro Lanche": {"F": 1, "L": 1},
    "Almoço": {"C2": 1, "G": 1, "V": 2, "P": 1, "S": 1},
    "Segundo Lanche": {"B": 1, "S": 1, "C1": 1},
...
prob += lpSum([(energia[i]) * food_vars[i] for i in food_items])
...
# Pesos
prob += lpSum([(pesos[f]) * food_vars[f] for f in food_items]) <= 5.0

# Energia(Calorias) - Variável
prob += lpSum([(energia[f]) * food_vars[f] for f in food_items]) >= 1800.0, "CaloriaMinima"
prob += lpSum([(energia[f]) * food_vars[f] for f in food_items]) <= 2200.0, "CaloriaMaxima"

# Carboidrato - Variável
prob += lpSum([(carboidrato[f]) * food_vars[f] for f in food_items]) >= 202.5, "CarboidratoMinimo"

# Sodio
prob += lpSum([(sodio[f]) * food_vars[f] for f in food_items]) <= 2000.0, "SodioMaximo"

# Proteinas
prob += lpSum([(proteina[f]) * food_vars[f] for f in food_items]) >= 28.4, "ProteinasMinimo"
prob += lpSum([(proteina[f]) * food_vars[f] for f in food_items]) <= 38.4, "ProteinasMaximo"

# Fibras
prob += lpSum([(fibra[f]) * food_vars[f] for f in food_items]) >= 30.0, "FibrasMinimo"

# Calcio
prob += lpSum([(calcio[f]) * food_vars[f] for f in food_items]) >= 850.0, "CalcioMinimo"

# Magnesio
prob += lpSum([(magnesio[f]) * food_vars[f] for f in food_items]) >= 400.0, "MagnesioMinimo"

# Fosforo
prob += lpSum([(fosforo[f]) * food_vars[f] for f in food_items]) >= 700.0, "FosforoMinimo"

# Ferro
prob += lpSum([(ferro[f]) * food_vars[f] for f in food_items]) >= 18.0, "FerroMinimo"

# Zinco
prob += lpSum([(zinco[f]) * food_vars[f] for f in food_items]) >= 15.0, "ZincoMinimo"
...
for refeicao in nomes_refeicoes:
    for classificacao_ref in contagem_classificacoes_necessarias[refeicao]:
        # Garanta que o número necessário de alimentos com a classificação específica seja selecionado para a refeição
        prob += lpSum([food_vars[f] for f in food_items if classificacao[f] == classificacao_ref]) >= contagem_classificacoes_necessarias[refeicao][classificacao_ref], "{}_{}_Min".format(refeicao, classificacao_ref)
...
for f in food_items:
    if food_vars[f].varValue > 0:
        print(f, food_vars[f].name, food_vars[f].varValue)

for refeicao in nomes_refeicoes:
    print("Alimentos para {}:".format(refeicao))
    for v in prob.variables():
        if v.name.startswith("{}_Selecao".format(refeicao.replace(" ", "_"))) and v.varValue > 0:
